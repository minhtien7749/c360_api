...
public class DepDltFactQuery {
    private final String SQL_TEMPLATE = "SELECT acn, "
            ...
            + "SUM(case when lnd.cob_dt >= (select MAX(cob_dt) FROM datalake.customer360.ETL_PROCESS_LOG WHERE job_name = 'c360_depst_dtls_fact_update_nii') then null else ytd_nii_alco_amt_in_lc end) AS ytdNiiAmt, "
            + "SUM(case when lnd.cob_dt >= (select MAX(cob_dt) FROM datalake.customer360.ETL_PROCESS_LOG WHERE job_name = 'c360_depst_dtls_fact_update_nii') then null else ytd_nii_alco_amt_in_lc end) / SUM(ytd_avg_ldgr_bal_amt_in_lc) AS ytdNimAmt, ";
    ...
    private String buildBaseSQL(String flag) {
        ...
        if (this.request.getPeriod() == null && hasToDate && !hasFromDate) { 
            sqlBuilder.append(" FROM datalake.customer360.c360_depst_dtls_fact lnd WHERE ")
                      .append(" lnd.cob_dt = ? ");
        }
        ...
        if(hasFromDate && !hasToDate) {
            sqlBuilder.append(" FROM datalake.customer360.c360_depst_dtls_fact lnd WHERE ")
            .append("lnd.cob_dt in (select cob_dt  from datalake.customer360.tm_dim where ")
            .append(flag).append(" = 'Y' AND (cob_dt >= ? AND cob_dt <= CURDATE()) or cob_dt = (SELECT MAX(cob_dt) FROM datalake.customer360.ETL_PROCESS_LOG WHERE job_name = 'c360_lon_dtls_fact')) ");
        }        
        if (this.request.getPeriod() != null && (hasFromDate && hasToDate)) {
        	 sqlBuilder.append(" FROM datalake.customer360.c360_depst_dtls_fact lnd WHERE ")
             .append("(lnd.cob_dt in (select cob_dt  from datalake.customer360.tm_dim where ")
